...
double I_ref;                                  //reference current [A]
...
int FB = 10;                                   //Fuzzy Logic Error Band (after which hyst. control takes over)[A]
...
void loop() {
  ...
  if (eI[2]>FB){                                //Current error lies within FL error band then F-controller is used
    CC->setInput(1,eI[2]);                      //Enter current error to fuzzy controller
